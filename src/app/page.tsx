...
export default function Home() {
  ...
  const { totalBillsAmount, totalExpenses, upcomingBillsCount, overdueBillsCount } = useMemo(() => {
    const now = new Date();
    return {
      totalBillsAmount: bills.reduce((acc, bill) => acc + bill.amount, 0),
      totalExpenses: bills
        .filter((b) => !b.isPaid)
      ...
      <main className="flex-1 p-4 md:p-8">
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4 mb-8">
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">₹{totalBillsAmount.toFixed(2)}</div>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
